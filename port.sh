...
echo "Setup complete. Access services from Windows browser using localhost:<port>. Press Ctrl+C to stop port forwarding."

# Trap Ctrl+C to clean up
trap 'echo "Stopping port forwarding..."; kill $PID1 $PID2 $PID3; wait $PID1 $PID2 $PID3 2>/dev/null || true; exit 0' INT
